...
class TennisTest(tennisTestCase:TennisTestCase) {

  def checkAllScores(game :TennisGame) {
    val highestScore = Math.max(tennisTestCase.player1Score, tennisTestCase.player2Score)
    for (i <- 0 until highestScore by 1) {
      if (i < tennisTestCase.player1Score)
        game.wonPoint("player1")
      if (i < tennisTestCase.player2Score)
...
object TennisTest {
  ...
  def  getAllScores: java.util.Collection[Array[TennisTestCase]] = {
    val list = new util.ArrayList[Array[TennisTestCase]]()
